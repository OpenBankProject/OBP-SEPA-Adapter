...
object WriteDatabase extends App {
  ...
  val customers = Seq(
    ...
    archives.Customer(UUID.randomUUID(), "Gustave", "Veilleux", "97, rue de la Boétie", "78370", "PLAISIR", "France", LocalDate.of(1998, Month.JANUARY, 3), "Paris", "France"),
    archives.Customer(UUID.randomUUID(), "Benjamin", "Blau", "Buelowstrasse 2", "56769", "Arbach", "Germany", LocalDate.of(1961, Month.MARCH, 26), "Frankfurt", "Germany")
  )
  ...
  val accounts = Seq(
    ...
    archives.Account(UUID.randomUUID(), Iban("DE74500105177926551355"), customers(1).id, AccountType.CURRENT, AccountStatus.ACTIVE, BigDecimal(100.0), BigDecimal(150.0)),
    archives.Account(UUID.randomUUID(), Iban("DE62500105177222842858"), customers(2).id, AccountType.CURRENT, AccountStatus.ACTIVE, BigDecimal(200.0), BigDecimal(200.0)),
    archives.Account(UUID.randomUUID(), Iban("DE49500105173712986459"), customers(2).id, AccountType.SAVING, AccountStatus.ACTIVE, BigDecimal(1000.0), BigDecimal(1000.0))
  )
  ...
  val beneficiaries = Seq(
    ...
    archives.Beneficiary(UUID.randomUUID(), customers(1).id, Iban("DE62500105177222842858"), Bic("OBPCDEB1XXX"), "Benjamin Blau", null),
    archives.Beneficiary(UUID.randomUUID(), customers(1).id, Iban("DE05500105179384878339"), Bic("NTSBDEB1XXX"), "Phillipp Kuefer", Some("My friend Phillipp")),
    archives.Beneficiary(UUID.randomUUID(), customers(2).id, Iban("DE42500105175212175614"), Bic("DEUTBEBEXXX"), "Lukas Kuhn", Some("Accommodation owner"))
  )
  ...
  val sddMandates = Seq(
    ...
    archives.SDDMandate(UUID.randomUUID(), "ABCD087488498", customers(2).id, accounts(2).id, "Deutsche Telekom", "DE03ABC484821", Iban("DE89500105179599249252"), Bic("DEUTBEBEXXX"), SDDMandateType.CORE, recurrent = true, SDDMandateStatus.ACTIVE, LocalDate.of(2019, Month.DECEMBER, 28)),
  )
  ...
  val files = Seq(
    ...
    archives.CoreBankingFile(UUID.randomUUID(), "CARD_CLEARING_2020-05-06", Path.of("files/archives/card/clearing"), CoreBankingFileType.CARD_CLEARING_FILE, CoreBankingFileStatus.PROCESSED, Some(LocalDateTime.of(2020, Month.MAY, 6, 2, 5, 51)), Some(LocalDateTime.of(2020, Month.MAY, 6, 2, 35, 24)), Some(21326)),
  )
  ...
  val cardTransactions = Seq(
    ...
    archives.CardTransaction(UUID.randomUUID(), accounts(1).id, TransactionType.CARD_CHIP_PAYMENT, TransactionStatus.NOT_CLEARED, BigDecimal(50), LocalDateTime.of(2020, Month.MARCH, 12, 17, 35, 21), accounts(1).balance + 50, accounts(1).balance, cards(1).id, "Restaurant LE MARTRAY", None, None),
    archives.CardTransaction(UUID.randomUUID(), accounts(2).id, TransactionType.CARD_NFC_PAYMENT, TransactionStatus.CLEARED, BigDecimal(20), LocalDateTime.of(2020, Month.MAY, 5, 8, 56, 48), accounts(2).balance + 20, accounts(1).balance, cards(2).id, "Super Shoes Store", Some(LocalDate.of(2020, Month.MAY, 6)), Some(files(1).id)),
  )
  ...
  val setup = DBIOAction.seq(
    SchemaArchive.customers ++= customers,
    SchemaArchive.accounts ++= accounts,
    SchemaArchive.beneficiaries ++= beneficiaries,
    SchemaArchive.sddMandates ++= sddMandates,
    SchemaArchive.cards ++= cards,
    SchemaArchive.coreBankingFiles ++= files,
    SchemaArchive.cardTransactions ++= cardTransactions,
    SchemaArchive.sepaTransactions ++= sepaTransactions
  )
